/*************************************************************************************/
/*Wrapper code that collapses levels and produces Weight of Evidence transformed variables. */
/*This code uses the macro from Lund and Raimi paper (Collapsing Levels of Predictor Variables for Logistic Regression*/
/*and Weight of Evidence Coding, http://www.mwsug.org/proceedings/2012/SA/MWSUG-2012-SA03.pdf)*/
%macro NOD_BIN_numeric_wrapper(
/*********************************************************************************/
/*Input*/
...
%do iter=1 %to &varnum.;
/*%let iter=2;*/
%let currvar = %scan(&vars_list.,&iter.);
%put The variable analysed is &currvar.;
proc means data=rank1 (keep= &target_variable. &currvar. &recoded_var_prefix.&iter.)  min max nway noprint;
	class &recoded_var_prefix.&iter.;
	var &currvar.;
...
run;
data rank1_means;
format original_var_name $32. recoded_var_name $32.;
label recoded_var_num=" " min_origv=" " max_origv=" ";
	set rank1_means (rename=(&recoded_var_prefix.&iter.=recoded_var_num));
	original_var_name = "&currvar.";
	recoded_var_name = "&recoded_var_prefix.&iter.";
run;
...
format variable $32.;
	set ___&recoded_var_prefix.&iter._woe&k.;
...
run;
proc append base=&output_original_recode_summary. data=Rank1_means force;
run;
%end;
